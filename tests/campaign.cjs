...
function pilot(){let phase='pick',target=null,timer=0,dir=1,demoReady=false,previousTime=0;return f=>{
 ...
 const u=f.u;let tx=u.x,ty=200,beam=true;
 if(f.blocked)return {x:0,y:-.5,beam:false};
 if(f.hits<f.mission.smash){
  if(!f.link){target=f.activeLoads().find(b=>b.kind==='cow')||f.activeLoads()[0];tx=target.x;ty=target.y-145;}
  else if(f.solids.length&&!demoReady){ty=190;tx=u.y>220?u.x:660;if(u.x>640&&u.y<220)demoReady=true;}else{const left=f.solids.length?660:120;tx=dir>0?1120:left;ty=425;if(u.x>1080)dir=-1;if(u.x<left+20)dir=1;}
 }else{
  ...
  if(phase==='return'){beam=false;ty=190;tx=u.y>220?u.x:target.x;if(Math.abs(u.x-target.x)<12&&u.y<220){phase='pick';}}
  if(phase==='pick'){tx=target.x;ty=target.y-145;if(f.link===target){phase='lift';timer=0;}}
  if(phase==='lift'){tx=target.x;ty=190;timer+=elapsed;if(u.y<215&&timer>2){phase='carry';timer=0;}}
  if(phase==='carry'){tx=f.pad.x+f.pad.w/2;ty=190;if(Math.abs(u.x-tx)<15&&Math.abs(target.x-tx)<35&&Math.abs(target.vx)<35){phase='lower';timer=0;}}
 ...
 return {x:Math.max(-1,Math.min(1,((tx-u.x)*3-u.vx*3)/500)),y:Math.max(-1,Math.min(1,((ty-u.y)*5-u.vy*4)/500-(f.link?f.link.m*260/2300:0))),beam};
};}
function run(index,fps=120){const f=new Field(index),input=pilot();for(let i=0;i<fps*300&&!f.won;i++)f.advance(1/fps,input(f));return f;}
...
const free=new Field(6);for(let i=0;i<1200;i++)free.step({});assert(!free.won,'practice never ends automatically');
